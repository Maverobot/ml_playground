...
# This code is inspired by https://github.com/llSourcell/Q_Learning_Explained/blob/master/q_learning.py
...
def run_train_episode(env,
    ...
    for t in range(t_max):
        ...
        policy[s1, s2, s3, s4, action] += learning_rate * (
            reward + gamma * np.max(policy[s1_, s2_, s3_, s4_]) -
            policy[s1, s2, s3, s4, action])
...
if __name__ == '__main__':
    ...
    for i in range(n_iterations):
        ...
        q_table, total_reward = run_train_episode(
            env,
            policy=q_table,
            n_buckets=n_buckets,
            render=True,
        ...
        plt.pause(0.0001)
        if i % 10 == 0:
            print('Iteration #%d -- Total reward = %d, learning rate = %f.' %
                  (i + 1, total_reward, learning_rate))

    solution_policy = np.argmax(q_table, axis=4)
    solution_policy_scores = [
        run_test_episode(env, solution_policy, n_buckets, False)
        for _ in range(100)
    ]
